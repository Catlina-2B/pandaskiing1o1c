...
import { useAllDeposits, useGlobalStats, formatAmount } from "@/hooks/useSubgraphData";
...
export default function PandaSkiingPage() {
  ...
  const { data: allDeposits, isLoading: depositsLoading, error: depositsError } = useAllDeposits(200); // 获取最近200笔充值用于绘制曲线
  
  // 优先使用合约数据，其次使用 subgraph 数据
  const contractStats = depositCount !== null && totalDeposited !== null ? {
  ...
  // 如果没有全局统计数据，从充值数据中计算
  const computedStats = allDeposits && allDeposits.length > 0 ? {
    totalDeposits: allDeposits.length.toString(),
    totalAmount: allDeposits.reduce((sum, deposit) => sum + parseFloat(deposit.amount), 0).toString(),
    uniqueDepositors: new Set(allDeposits.map(d => d.depositor)).size.toString(),
    totalWithdrawn: "0"
  } : null;
  ...
  // 判断是否显示高价提示
  const currentPrice = totalDeposited !== null 
  ...
  return (
    ...
    >
        ...
        <div className={`fixed top-0 left-0 right-0 z-40 border-b ${
          ...
          <div className="container mx-auto px-4 py-3">
            <div className={`font-mono text-xs md:text-sm leading-relaxed ${
              ...
              Look, beyond ${HIGH_PRICE_THRESHOLD}, the curve doesn't matter, that's fucking too high for most of ppl. If you still want in at that level, we should just talk directly. Same benefits. Same circle. But no shares, share may be something, not guarantee. 
              <span className="block md:inline mt-1 md:mt-0 md:ml-2">
                Send <span className="font-bold">42.69 USDT</span> (BSC): 
                <span className={`ml-1 font-mono text-[10px] md:text-xs ${
      ...
      {/* 主要内容 */}
      <div className={`min-h-screen flex flex-col lg:flex-row relative z-10 transition-all duration-300 ${
        showHighPriceAlert ? "pt-16 md:pt-20" : ""
      }`}>
        {/* 移动端和PC端右侧：介绍内容 */}
        <div className="flex-1 lg:order-2 p-4 lg:p-8 flex flex-col min-h-screen">
          <div className="w-full">
                ...
                >
                  <div className="px-4 py-2 space-y-4">
                    ...
                    >
                      ...
                      <p className={isDark ? "text-green-400/70" : "text-gray-600"}>
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        <p className="leading-relaxed text-sm">
                          1 对 1 Call 不涉及 NDA 的部分，我知无不言。然后拉小群一起讨论。我也会把朋友拉进来：一线
                    ...
                    >
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <p className="mt-4 font-medium">让我们一起试着做些改变吧！</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-8 text-center lg:text-left">
              <Button
                className={`px-6 py-2 border font-mono text-xs transition-all duration-200 ${
                  isDark
                    ? "bg-black border-green-400 text-green-400 hover:bg-green-400/10"
                    : "bg-white border-gray-900 text-gray-900 hover:bg-gray-900/5"
                }`}
              ...
              {isLoadingNextAmount && isConnected && (
                <div className={`mt-3 text-xs font-mono ${isDark ? "text-green-400/70" : "text-gray-600"}`}>
                  正在加载充值信息...
            </div>
              )}
              ...
              {isConnected && !isLoadingNextAmount && !nextDepositAmount && (
                <div className={`mt-3 p-3 border font-mono text-xs ${
                  isDark 
              ...
              {/* 显示下一次充值金额 */}
              {isConnected && nextDepositAmount !== null && nextDepositAmount > 0n && (
                <div className={`mt-3 text-xs font-mono ${isDark ? "text-green-400/70" : "text-gray-600"}`}>
              ...
              {isConnected && tokenBalance !== null && tokenBalance >= 0n && (
                <div className={`mt-1 text-xs font-mono ${isDark ? "text-green-400/70" : "text-gray-600"}`}>
        ...
        {/* PC端左侧：价格图表区域 */}
        <div className="lg:w-1/2 xl:w-2/5 lg:order-1 p-4 lg:p-8 flex flex-col">
          <div
          ...
          >
            ...
            >
              ...
              >
                  ...
                  <>
                    <path
                      d={(() => {
                        ...
                        const maxAmount = Math.max(...deposits.map(d => parseFloat(d.amount) / 1e18));
                        let pathData = '';
                        const pointCount = deposits.length;
                        const xSpacing = pointCount > 1 ? 680 / (pointCount - 1) : 680;
                ...
                {/* 数据点 - 使用真实充值数据 */}
                <g>
                  {allDeposits && allDeposits.length > 0 && allDeposits.map((deposit, i) => {
                    const pointCount = allDeposits.length;
                    const xSpacing = pointCount > 1 ? 680 / (pointCount - 1) : 680;
                    const x = 80 + (i * xSpacing); // 在图表宽度内均匀分布
                    const amount = parseFloat(deposit.amount) / 1e18;
                    const maxAmount = Math.max(...allDeposits.map(d => parseFloat(d.amount) / 1e18));
                ...
                <g className="font-mono" style={{ fontSize: "10px" }}>
                  {allDeposits && allDeposits.length > 0 && (() => {
                    ...
                    const labelIndices = Array.from({ length: maxLabels }, (_, i) => 
                      Math.min(i * step, allDeposits.length - 1)
                    );
                    const pointCount = allDeposits.length;
                    const xSpacing = pointCount > 1 ? 680 / (pointCount - 1) : 680;
                    ...
                    return labelIndices.map((idx) => {
                      ...
                      const time = new Date(parseInt(deposit.timestamp) * 1000).toLocaleTimeString('zh-CN', {
                        hour: '2-digit',
                        minute: '2-digit'
                      });
              ...
              >
                ...
                >
                  ...
                  {contractStats && (
                    <span className={`text-[10px] ${isDark ? "text-green-400/60" : "text-gray-600"}`}>
                      [链上数据]
                    </span>
                  )}
                </div>
                <div className="p-2">
                  {totalDeposited !== null ? (
                    <>
                      <div className="text-sm font-bold">{formatUnits(totalDeposited, 18)} USDT</div>
                      <div className="text-xs">总充值金额</div>
                      <div className={`text-[10px] mt-1 ${isDark ? "text-green-400/60" : "text-gray-600"}`}>
                        {depositCount?.toString() || "0"} 次充值
                      </div>
                    </>
                  ) : stats ? (
                    <>
                      <div className="text-sm font-bold">{formatAmount(stats.totalAmount)}</div>
                      <div className="text-xs">总充值金额</div>
                    </>
                  ) : (
                    <>
